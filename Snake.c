...
void gameOver(){
	//Sound_Death();
	for(int i = 0; i < 12000000;i++){} //INSERT DEATH SONG
...
void SysTick_Init(int32_t speed){
	NVIC_ST_CTRL_R = 0;
	NVIC_ST_RELOAD_R = (-50 * speed * (speed / 7)) + 2430000000;
	NVIC_ST_CURRENT_R = 0;
	NVIC_SYS_PRI3_R = (NVIC_SYS_PRI3_R&0x00FFFFFF)|0x40000000; // priority 2
	NVIC_ST_CTRL_R |= 0x07;
}

/**	Change snake speed based on ADC input
*/
void changeSpeed(){
...
void move(){		
	...
	if(temp.row > 17 || temp.col > 15)			//Look if out of boundary occurs
		gameOver();
